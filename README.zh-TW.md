...
- ✓ `--pending` 用那份本機紀錄回答「還有哪些包裹在路上」，完全不連線——顯示每件包裹上次回報的狀態，以及那是多久以前的事
- ✓ `--refresh-pending` 回答「所有未結案包裹**現在**的狀態」——對每個未結案單號逐一即時查詢、更新本機紀錄，並印出依結果分類的彙整（新結案／有新進度／無變化／已略過／查詢失敗）
...
python3 scripts/track.py --pending                       # 看哪些還在路上
python3 scripts/track.py --refresh-pending                # 對未結案包裹逐一即時查詢
...
`--pending` 回答的是另一種問題——不是「這件到哪了」，而是「我還有幾件在路上」：
...
這份清單完全來自本機紀錄，**不會發出任何請求**，所以顯示的是每件包裹「上次查詢時」的狀態，
而 `（N 天沒更新）` 是最後一筆事件距今多久。要知道現在到哪了，就拿那個單號再查一次——
或者一次把全部未結案的都更新：
...
`--refresh-pending` 走的是 `--pending` 讀的同一份本機清單，但不是印快照，而是對每個單號逐一
即時查詢——一次一筆、走跟單號查詢相同的路徑——再依結果分類成彙整：**新結案**（這次查到已送達）、
**有新進度**（最新事件變了但還沒完成）、**無變化**、**已略過**、**查詢失敗**。查成功的那筆會
覆寫該號的本機快照，機制跟一般查詢寫入完全相同（`--no-record` 一樣能抑制寫入）。某一家掛掉不會
讓整批中止——每筆自己的結果（含錯誤原因）就顯示在那一行。如果某筆未結案紀錄的貨運需要
[17TRACK 橋接](#17track選配)但沒設 key，會被列為**已略過**並附上原因，不算失敗——沒設 key
就不會真的呼叫，那不是這個指令該替使用者決定的事。

`--pending` 與 `--refresh-pending` 刻意回答不同問題：`--pending` 秒回且完全免費，因為它不離開
你的電腦；`--refresh-pending` 每筆未結案包裹要花一次請求，包裹愈多就愈久，因為它是真的再問一次
每家貨運公司。

在 Claude Code 裡不需要自己下指令——直接說「查一下這個包裹 900000000001」或
「還有哪些包裹在路上」即可；想要現況而不是上次的快照，就說「幫我更新一下未結案包裹」。
...
- 適用於**個人低頻**查詢。無重試迴圈、無並行；請求 10 秒逾時後直接失敗。請勿拿來批次跑單號清單。
  `--refresh-pending` 也不例外——它對未結案清單一樣是逐一序列查詢，未結案筆數愈多就單純愈久。
